...
if exists('*entry2#init')
    finish
endif
" set shortmess+=W
" set shortmess+=filmnrwxaoOstTWAIcqFS

func! entry2#init() abort
endfunc " entry2#init
...
let idx = match(&rtp, ',')
let $ROOT = &rtp[:idx-1] . '/'
let &rtp = $ROOT . 'vim/dev,' . &rtp[idx+1:]
let $depth = $depth + 1
set isfname=@,48-57,/,.,-,_,~,$
nn q :q<CR>
nn gt <C-^>
...
au BufNew * if isdirectory(expand('<amatch>')) | call dirbuf#openpath(expand('<amatch>'),0,0) | endif
