...
" no need to copy files, replace rtp[0] with dev/.
" mv dev/plugin to main/plugin, dev/ only has autoload/ ft/ etc.
if exists('*entry3#init')
    finish
endif
" set shortmess+=W
" set shortmess+=filmnrwxaoOstTWAIcqFS

func! entry3#init() abort
endfunc " entry3#init
...
au BufEnter * ++once if isdirectory(bufname('%')) | call dirbuf#openpath(bufname('%')) | endif
func! s:fix_dir(nr, name) abort
    if !isdirectory(a:name) | return | endif
    exec 'au BufEnter <buffer=' . (a:nr+0) . '> ++once set nobuflisted'
    call dirbuf#openpath(a:name,0,0)
endfunc " s:fix_dir
au BufNew * call s:fix_dir(expand('<abuf>'), expand('<amatch>'))
au VimEnter * call timer_start(400, {timer -> execute('runtime ft/ftdetect.vim')})

nn ;s :<C-u>exec 'b '. buf#nextscratch(v:count)<CR>
nnoremap <F4> :<C-u>call buf#Toggle_diff_alternate_buffer()<CR>
autocmd GUIEnter * runtime platform/gvim.vim
if has('win32') | runtime platform/win32.vim | endif
...
" set mouse=a     " useful in nested case.
set history=10000
set encoding=utf-8
...
hi ErrorMsg term=standout ctermfg=15

	let g:loaded_netrw       = 1
	let g:loaded_netrwPlugin = 1
let g:default_action = -1
" so ~/notes/.vimrc
" let g:ackprg = 'ag'
        " \ ['ack.vim', 0],
let g:bundle_ignore = [
        \ 'vim-easymotion', 'indent-object.vim', 'vim-dirvish',
        \ ['dev', -1],
        \ ['main', [0.3,5.2]],
        \ ['YouCompleteMe', 5],
        \ ['vim-fugitive', 1.2],
...
call bundle2#init($ROOT . "vim")
call bundle2#start('')   " same as call bundle#init()  + call bundle#start()
finish
