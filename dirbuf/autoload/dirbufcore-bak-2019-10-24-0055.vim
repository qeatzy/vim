...
func! s:add(path)
    ...
    if empty(d) || updated
        ...
        if empty(dirs)
            let tree_cache = files
        else
            for name in dirs
                let subdir = get(g:dirs, base . name)
                ...
                if type(subdir) == v:t_number
                    ...
                    if type(tree_cache) != v:t_number   " tree cached
                        let prefix = name . '/'
                        let tree[name] = empty(tree_cache) ? [name] : 
                                \ map(copy(tree_cache), 'prefix . v:val')
                    elseif !empty(subdir['not_tree_builded'])
        ...
        let d['dirs'] = dirs
        let d['files'] = files
        let d['not_tree_builded'] = not_tree_builded
        let d['tree'] = tree
        if exists('tree_cache')
